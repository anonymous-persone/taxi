	...
	<div class="col-md-12">
        <div class="card table-card latest-activity-card">
            <div class="card-header">
				<h5>{{__($title)}}</h5>
                @if ($user->able(16))
				<button id="add-new" class="btn btn-primary waves-effect waves-light float-right"><i class="feather icon-plus"></i> {{__('Add New Cards')}}</button>
                @endif         
                ...
                <div class="table-responsive">
                    <table id="dataTable" class="table table-hover table-borderless tablesorter">
                        <thead class="border-checkbox-section">
                            <th id="cardNumber">{{__('Card Number')}}</th>
                            <th id="redeemedBy">{{__('Redeemed By')}}</th>
                            <th id="value">{{__('Value')}}</th>
                            {{-- <th>{{__('Actions')}}</th> --}}
                        </thead>
...
<script type="text/javascript" charset="utf8" src="https://cdn.datatables.net/1.10.19/js/jquery.dataTables.js"></script>
    ...
    <script type="text/javascript">
        $("#add-new").on('click', function(){
            document.location.href = "{{route('cards.add')}}";
        })
    </script>
    <script type="text/javascript">
