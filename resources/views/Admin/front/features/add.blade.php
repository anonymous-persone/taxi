...
@section('page_title', __('Add new feature'))

@section('page_body')
<form id="pageForm" method="post" action="{{@route('feature.create')}}" class="form-material" enctype="multipart/form-data">
    <div class="row">
    	<div class="col-md-9">
            <div class="card table-card">
                <div class="card-header">
                    <h5>{{__('Add new feature')}}</h5>
                </div>
                <div class="card-block" id="card-block">
                    <div class="form-group form-primary">
                        <input type="text" autocomplete="off" name="title" id="title" class="form-control" required="" value="{{Request::old('title')}}">
                        <span class="form-bar"></span>
                        <label class="float-label">{{__('Title')}}</label>
                    </div>
